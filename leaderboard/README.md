...
- M16 clears have optional `clearTimeMs` (total run time through the killing blow) and `durationMs = 900000` (night survived). Clears rank first, fastest total time first; survival-only records follow, longest first. The combined list is capped at 100 per version. Missing clear metadata preserves historical survival records. A first/faster clear replaces a device's previous record; survival-only submissions cannot overwrite a clear. Equal values use index creation-time ordering and ranks remain sequential.
