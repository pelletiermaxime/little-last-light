...
- New records snapshot `energyInvested` (energy spent on the defense at run start), `energyEarned` (earnings from this run), and `turretLayout`. Investment is the current layout's purchase cost: the starting turret is free, and subsequent turrets cost 20, 30, 40, etc. It excludes unspent savings, refunded past layouts, and earnings generated during the run. This gives survival records context about how much progression supported the defense. The layout contains arena `width`/`height` and normalized `{ x, y }` turret positions in the range 0–1, with the origin at the top left. All current turrets have the same configuration, so positions fully describe the defense pattern. The website renders a responsive SVG diagram at the saved arena aspect ratio; no screenshot upload or image storage is needed. The center marker is an arena reference, not a recorded player position.
