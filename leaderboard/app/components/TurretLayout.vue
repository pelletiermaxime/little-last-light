...
<template>
  <figure class="turret-layout">
    <svg :viewBox="`0 0 ${layout.width} ${layout.height}`" role="img" :aria-label="`${username}'s run layout: ${layout.turrets.length} turrets`">
      ...
      <desc>Top-down arena. Blue circles mark damage turrets, purple diamonds mark slow turrets, and gray circles mark turrets whose type was not recorded. The cross marks the arena center.</desc>
      ...
      <g v-for="(turret, index) in layout.turrets" :key="index" :transform="`translate(${turret.x * layout.width} ${turret.y * layout.height})`">
        <title>{{ turret.type === 'pulse' ? 'Slow turret' : turret.type === 'damage' ? 'Damage turret' : 'Turret · Type not recorded' }}</title>
    ...
    <figcaption>
      ...
      <div class="legend"><span v-for="item in legend" :key="item.label"><span class="turret-dot" :class="{ diamond: item.type === 'pulse' }" :style="{ background: item.color }" aria-hidden="true" /> {{ item.count }} {{ item.label }}</span></div>
