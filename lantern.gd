...
func _process(delta: float) -> void:
	...
	var base_rate: float = ENERGY_RATES[brightness] * get_parent().energy_multiplier()
	# Split the expiry frame: Kindling doubles only its remaining active seconds.
	energy += base_rate * (delta + get_parent().pickups.consume_kindling(delta))
	elapsed += delta
...
func current_energy_rate() -> float:
	...
	return ENERGY_RATES[brightness] * get_parent().energy_multiplier() * bonus
