...
func check() -> void:
	...
	assert(prep.upgrade_groups.get_child_count() == 3 and prep.upgrade_progress.size() == 6)
	...
	joy(JOY_BUTTON_A)
	assert(scene.slow_levels.slow_rate == 1 and scene.banked_energy == 9940.0, "Controller buys exactly one level")
	assert(prep.upgrade_summaries["Slow turrets"].text.contains("2.7s"))
	assert(prep.upgrade_progress.slow_rate.get_child(4).text.strip_edges() == "1/4")
	joy(JOY_BUTTON_B)
	...
	assert(is_equal_approx(pulse.fire_interval, 2.7), "New turrets inherit previously bought upgrades")
	assert(scene.buy_upgrade("slow_strength") and scene.buy_upgrade("slow_duration"))
	assert(is_equal_approx(pulse.slow_factor, 0.5) and is_equal_approx(pulse.slow_duration, 1.7), "Existing turrets update immediately")
	...
	assert(scene.defense_investment() == 230.0, "Investment includes 190 upgrades and 40 turret cost")
	...
	assert(is_equal_approx(pulse.fire_interval, 1.8) and is_equal_approx(pulse.slow_factor, 0.35) and is_equal_approx(pulse.slow_duration, 2.3))
	prep.open_view(prep.View.UPGRADES)
	for button in prep.slow_buttons.values():
		assert(button.disabled and button.text.contains("MAX") and not button.text.contains("→"))
	for kind in scene.slow_levels:
		assert(prep.upgrade_progress[kind].get_child(4).text.strip_edges() == "4/4")
	scene.save_progress()
	var data: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(path))
	for kind in scene.slow_levels:
		for invalid in [-1, 1.5, 5, "2"]:
	...
	assert(scene.slow_levels == {"slow_rate": 4, "slow_strength": 4, "slow_duration": 4})
	assert(is_equal_approx(pulse.fire_interval, 1.8) and is_equal_approx(pulse.slow_factor, 0.35) and is_equal_approx(pulse.slow_duration, 2.3))
	scene.start_run()
	...
	enemy.position = pulse.position + Vector2(30, 0)
	pulse._process(0.01)
	assert(is_equal_approx(enemy.slow_factor, 0.35) and is_equal_approx(enemy.slow_remaining, 2.3), "Actual pulse uses upgraded strength and duration")
	enemy._consume_slow(1.8)
	assert(enemy.slow_remaining > 0)
	pulse._process(1.8)
	assert(is_equal_approx(enemy.slow_remaining, 2.3) and is_equal_approx(enemy.slow_factor, 0.35), "Maximum rate can refresh the effect before expiry without stacking")
	...
	assert(scene.slow_levels.slow_strength == 4, "Layout refund preserves upgrades")
	scene.free()
	await process_frame
	...
	scene = game()
	pulse = get_nodes_in_group("turrets").back()
	assert(scene.slow_levels.slow_rate == 0 and pulse.fire_interval == 3.0 and is_equal_approx(pulse.slow_factor, 0.55) and pulse.slow_duration == 1.5)
	scene.free()
