...
func check() -> void:
	...
	expect(build.placing and build.preview.turret_type == "pulse" and build.preview.attack_range == 150.0, "Pulse button previews its actual type and range")
	...
	expect(pulse.turret_type == "pulse" and pulse.purchase_cost == 40.0 and scene.banked_energy == 960.0, "Pulse premium is charged and recorded")
	expect(build.turret_cost() == 30.0 and build.turret_cost("pulse") == 50.0, "Both types share layout price escalation")
	...
	build.begin_move(pulse)
	expect(build.try_place(Vector2(200, 100)) and scene.banked_energy == 960.0, "Moving a pulse is free")
	...
	expect(data.turret_types == ["damage", "pulse"] and data.turret_costs == [0.0, 40.0], "Save records mixed types and prices")
	...
	expect(pulse.turret_type == "pulse" and pulse.position.is_equal_approx(Vector2(200, 100)) and pulse.purchase_cost == 40.0, "Mixed layout reloads with exact price and location")
	scene.start_run()
	...
	build.begin_move(pulse)
	expect(build.sell_selected_turret() and scene.banked_energy == funds + 40.0, "Selling refunds original pulse price after reopening")
